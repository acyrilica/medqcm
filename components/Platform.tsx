...
const quotes = [
  ...
  "Le succès appartient à ceux qui préparent le mieux. 🎯",
  "Aujourd'hui étudiant, demain médecin. 🏥",
  "La persévérance est la clé de toute réussite médicale. ⭐",
  "Un QCM à la fois, une carrière se construit. 📚",
  "Les grands médecins ont tous commencé par réviser. 🌟",
...
function Quiz({ config, setPage, userId, bookmarkedIds, toggleBookmark }: { config: QuizConfig; setPage: (p: string) => void; userId: string; bookmarkedIds: Set<number>; toggleBookmark: (id: number) => void }) {
  ...
  const toggle = (oi: number) => {
    ...
    setSelections((prev: Record<number, Set<number>>) => {
      ...
      if (isMulti) { next.has(oi) ? next.delete(oi) : next.add(oi); } else { next.clear(); next.add(oi); }
